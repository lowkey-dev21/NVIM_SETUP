...
local function bufferline()
  ...
  local _, current_idx = utils.find(function(component)
...
local function setup_autocommands(conf)
  ...
  api.nvim_create_autocmd("User", {
    ...
    callback = function(args)
      ui.on_hover_over(args.buf, args.data)
    end,
  })
...
local function command(name, cmd, opts)
  api.nvim_create_user_command(name, cmd, opts or {})
end

local function setup_commands()
  command("BufferLinePick", function()
    M.pick()
  end)
  command("BufferLinePickClose", function()
    M.close_with_pick()
  end)
  command("BufferLineCycleNext", function()
    M.cycle(1)
  end)
  command("BufferLineCyclePrev", function()
    M.cycle(-1)
  end)
  command("BufferLineCloseRight", function()
    M.close_in_direction("right")
  end)
  command("BufferLineCloseLeft", function()
    M.close_in_direction("left")
  end)
  command("BufferLineCloseOthers", function()
    M.close_others()
  end)
  command("BufferLineMoveNext", function()
    M.move(1)
  end)
  command("BufferLineMovePrev", function()
    M.move(-1)
  end)
  command("BufferLineSortByExtension", function()
    M.sort_by("extension")
  end)
  command("BufferLineSortByDirectory", function()
    M.sort_by("directory")
  end)
  command("BufferLineSortByRelativeDirectory", function()
    M.sort_by("relative_directory")
  end)
  command("BufferLineSortByTabs", function()
    M.sort_by("tabs")
  end)
  command("BufferLineGoToBuffer", function(opts)
    M.go_to(opts.args)
  end, { nargs = 1 })
  command("BufferLineTogglePin", function()
    groups.toggle_pin()
  end, { nargs = 0 })
  command("BufferLineTabRename", function(opts)
    M.rename_tab(opts.fargs)
  end, { nargs = "*" })
  command("BufferLineGroupClose", function(opts)
    groups.action(opts.args, "close")
  end, {
    nargs = 1,
